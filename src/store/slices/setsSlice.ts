...
export const {setAdded, setUpdated, setRemoved, allSetsRemoved, manySetsRemoved} = setsSlice.actions;
...
export const selectSetIdsByRecipientId = (id: string) => {
    return createSelector(
        selectSets,
        (sets) => sets.filter((set) => set.recipient_id === id).map((set) => set.id)
    )
};
...
export const selectSetsByCollectionId = (id: string) => {
    return createSelector(
        selectSets,
        (sets) => sets.filter((set) => set.collection_id === id)
    )
};

export const selectSetIdsByCollectionId = (id: string) => {
    return createSelector(
        selectSetsByCollectionId(id),
